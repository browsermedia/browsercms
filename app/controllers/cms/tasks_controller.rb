module Cms
  class TasksController < Cms::BaseController
    ...
    def new
      @task = @page.tasks.build(:assigned_by => current_cms_user)
    end

    def create
      @task = @page.tasks.build(task_params())
      @task.assigned_by = current_cms_user
      if @task.save
    ...
    def complete
      if params[:task_ids]
        Task.where(["id in (?)", params[:task_ids]]).each do |t|
          if t.assigned_to == current_cms_user
            t.mark_as_complete!
          end
        end
        ...
        if @task.assigned_to == current_cms_user
