...
describe FileBlock do
  ...
  describe "when saving a new record" do
    ...
    describe "without a file_name" do
      it "should add an error" do
        f = new_file_block(:file => @file, :section => root_section)
        f.should_not be_valid
        f.errors.on(:file_name).should == "can't be blank"
      end
    end

    it "should create Attachment when passed a rails file object" do
      f = create_file_block(:file => @file, :file_name => "/test.jpg", :section => root_section)
    ...
    it "should use / when in the root section" do
      f = create_file_block(:file => @file, :file_name => "/test.jpg", :section => root_section)
      f.path.should == "/test.jpg"
    end

    it "should file path should not include section name" do
      s = create_section(:path => "/section_name")
      f = create_file_block(:file => @file, :file_name => "/test.jpg", :section => s)
    ...
    it "should return correct section id after loading from database" do
      f = create_file_block(:file => @file, :file_name => "/test.jpg", :section_id => root_section.id)
      f = FileBlock.find(f.id)
  ...
  describe "when updating an existing record" do
    before do
      @file_block = create_file_block(:section => root_section, :file_name => "/test.jpg", :file => mock_file, :name => "Test")
